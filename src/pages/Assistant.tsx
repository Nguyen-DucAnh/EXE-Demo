    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="space-y-4 text-center py-6">
        <div className="flex items-center justify-center gap-3">
          ...
          <h1 className="text-4xl font-extrabold bg-gradient-to-r from-sky-700 via-indigo-600 to-amber-500 bg-clip-text text-transparent">
            Trợ lý AI - HeyPaaaa
          </h1>
        </div>
      ...
      <Card className="h-[600px] flex flex-col border-2 border-sky-100 bg-gradient-to-br from-white to-sky-50/30 shadow-lg">
        <CardHeader className="border-b-2 border-sky-100 bg-gradient-to-r from-sky-50 to-amber-50">
          <CardTitle className="text-xl flex items-center gap-2">
            <MessageCircle className="h-5 w-5 text-primary" />
            Cuộc trò chuyện
          </CardTitle>
        </CardHeader>
        <CardContent className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-white to-sky-50/20">
          {questions.length === 0 ? (
            <div className="text-center text-gray-500 py-16">
              <div className="relative inline-block mb-6">
                <div className="absolute inset-0 bg-gradient-to-r from-sky-200 to-amber-200 rounded-full blur-lg opacity-30"></div>
                <MessageCircle className="h-16 w-16 mx-auto relative z-10 text-sky-400" />
              </div>
            ...
            questions.map((item, index) => (
              <div key={index} className="space-y-3">
                <div className="flex justify-end">
                  <div className="bg-gradient-to-r from-sky-600 to-indigo-600 text-white rounded-2xl rounded-tr-sm px-5 py-3 max-w-[80%] shadow-md">
                ...
                {item.answer && (
                  <div className="flex justify-start">
                    <div className="bg-white border-2 border-sky-100 rounded-2xl rounded-tl-sm px-5 py-3 max-w-[80%] shadow-sm">
                      <p className="text-xs font-semibold text-sky-700 mb-2 flex items-center gap-2">
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-white border-2 border-sky-100 rounded-2xl rounded-tl-sm px-5 py-3 shadow-sm">
        ...
        <div className="border-t-2 border-sky-100 p-4 bg-gradient-to-r from-sky-50/50 to-amber-50/50">
          <form onSubmit={handleSubmit} className="flex gap-3">
            <Textarea
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="💬 Đặt câu hỏi của bạn về chăm sóc vợ và con..."
              className="min-h-[90px] resize-none border-2 border-sky-200 focus:border-sky-400 rounded-xl text-base"
              disabled={loading}
            />
            <div className="flex flex-col gap-2">
              <Button
                ...
                size="icon"
                className="h-[90px] w-[90px] bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 shadow-lg hover:shadow-xl transition-all rounded-xl"
              >
                {loading ? (
